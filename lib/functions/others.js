...
exports.toLargeNumber = (nbr) => {
	...
	let Finish_Result = Formated_Number.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".").toString('');
	if (Finish_Result.length > 30 && Finish_Result.slice(30).split("").some(h => h == '0')) return Finish_Result.toString().slice(0, 30) + '... [ +' + Finish_Result.slice(30).length + ' Zeros 0 ]'; // Evitar que vire uma trava
