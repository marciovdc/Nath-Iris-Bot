...
exports.welcome = async (name, disc, guild, member, avatar, type, obj) => {
	const welni = type == 'add' ? 'Welcomer' : 'Leaver';
	const welc = await new canvacord[welni]()
	.setUsername(name)
	...
	if (obj.Use_Default !== true) {
		const Welver = type == 'add' ? '_Welcome' : '_GoodBye';
		...
		if (obj.User_Opacity !== null) {
			welc.setOpacity("username-box", Number(obj.User_Opacity));
		}
		if (obj.Disc_Opacity !== null) {
			welc.setOpacity("discriminator-box", Number(obj.Disc_Opacity));
		}
		if (obj.Message_Opacity !== null) {
			welc.setOpacity("message-box", Number(obj.Message_Opacity));
		}
		if (obj.Border_Opacity !== null) {
			welc.setOpacity("border", Number(obj.Border_Opacity));
		}
